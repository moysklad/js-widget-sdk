...
Для витрины приложений МойСклад можно включить автоматическую отправку высоты iframe на экземпляре SDK:
```
const sdk = WidgetSDK.create();
const stopExpand = sdk.autoResizeIframe({
  intervalMs: 250
});
```
...
- `stopExpand()` снимает `load`-слушатель и останавливает опрос;
