...
Хост отправляет `Open` сразу после загрузки iframe — как правило, раньше, чем виджет
смонтирует интерфейс и подпишется. Поэтому последнее полученное `Open` доигрывается
подписчику, который подписался позже: обработчик `Open` можно вешать в любой момент
(например, из `useEffect` в React), но он должен быть идемпотентным — при повторной
подписке он получит то же сообщение ещё раз.
...
- `onOpen` — событие `Open` (открытие/контекст виджета); последнее `Open` доигрывается поздним подписчикам.
