...
describe('WidgetSDKInstance basics', () => {
  ...
  test('_nextMessageId is monotonic', () => {
    const sdk = window.WidgetSDK.create({ debug: true });
...
describe('_handleMessage and events', () => {
  test('logs and ignores non-object messages', () => {
    const sdk = window.WidgetSDK.create({ debug: true });
  ...
  test('resolves pending request when matching correlationId arrives', async () => {
    const sdk = window.WidgetSDK.create({ debug: true });

    const promise = sdk.sendRequest({ name: 'TestRequest' });
    const messageId = sdk._requestIdCounter;

    const response = { correlationId: messageId, name: 'Response', payload: 42 };
  ...
  test('rejects pending request with InvalidMessageError', async () => {
    const sdk = window.WidgetSDK.create({ debug: true });
...
describe('sendRequest and sendMessage', () => {
  ...
  test('sendMessage catches postMessage errors and logs warning', () => {
    const originalParent = global.parent;
    const fakeParent = {
      postMessage() {
    ...
    global.parent = fakeParent;

    const sdk = window.WidgetSDK.create({ debug: true });
    const logSpy = jest.spyOn(sdk, '_log');

    sdk.sendMessage({ name: 'BrokenMessage' });
  ...
  test('sendRequest rejects when postMessage throws', async () => {
    ...
    const sdk = window.WidgetSDK.create({ debug: true });
    const promise = sdk.sendRequest({ name: 'WillFail' });
...
describe('high-level API helpers', () => {
...
describe('openFeedback / setDirty / clearDirty', () => {
